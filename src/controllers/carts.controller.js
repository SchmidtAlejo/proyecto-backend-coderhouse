...
class CartsController {
  ...
  static purchase = async (req, res) => {
    try {
      ...
      if (products.length === 0) throw new Error("No products in cart");
      let amount = 0;
      let productsNoStock = [];
      let productsStock = [];
      for (const product of products) {
        ...
        if (prd.stock < product.quantity) {
          ...
          productsStock.push({ product: prd, quantity: product.quantity });
        }
      }
      const ticket = {
        code: uuidv4(),
      ...
      productsStock.forEach(async (item) => {
        ProductsService.updateProduct(item.product._id.toString(), { stock: item.product.stock - item.quantity });
      })

      transporter.sendMail({
        to: req.user.email,
        subject: 'Compra realizada',
        html: `<p>Ticket ID: ${response._id}</p>
              <p>Total: $${amount}</p>`
      });
  ...
  static deleteAllCarts = async (req, res) => {
    try {
      await CartsService.deleteAllCarts()
      res.success("All carts deleted");
    } catch (error) {
      res.badRequest(error.message);
    }

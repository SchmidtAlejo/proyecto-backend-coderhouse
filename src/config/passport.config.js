...
import { coockieExtractror, createHash, isValidPassword } from '../utils/utils.js';
...
const passportInit = () => {
    ...
    passport.use('login', new LocalStrategy({ usernameField: 'email' }, async (username, password, done) => {
        try {
            const user = await modelUser.findOne({ email: username }).lean();
            if (!user) {
                return done(null, false);
            }
        ...
        new GitHubStategy.Strategy(
            {
                ...
                callbackURL: `${config.API_URL}/api/sessions/callbackGithub`,
            },
